...
- **stars 是关注度指标**:它不能直接代表质量、用户数或严格增长率。正文中的 stars 多用近似展示;精确值见原始数据。
...
### 指标读法:stars 是入口指标,不是质量结论

本文把 stars 当作注意力和传播结果,不把它直接等同于代码质量、真实用户数或商业价值。质量需要结合维护节奏、issue/PR 处理、contributors、测试、CI、release、许可证和真实使用反馈判断。当前数据已经包含 stars、forks、open issues、创建时间、最近 push 时间和 license;PR 数、merged PR、contributors、release cadence 还需要另行抓取。

| 指标 | 本文怎么用 | 不能直接说明什么 |
|---|---|---|
| `stars` | 注意力、收藏、传播结果 | 代码质量、真实使用量、收入 |
| `stars/day` | 6 个月窗口内的传播速度 | 长期留存和持续增长 |
...
在这批 703 个仓库里,stars 和 forks 的相关性较高(raw corr ~0.80,log corr ~0.76),所以 raw forks 大体会跟着 stars 走。更值得看的指标是 `fork/star`:全样本 median 约 10.4%,p75 约 14.0%,p95 约 29.0%。超过 30% 的项目通常带有教程、可运行替代、本地部署或工作流模板属性,更像“被拿去复制”的仓库。

可以把这些指标合成一个“质量不确定性”读法:先用 `stars/day` 判断项目是否进入传播层,再用 `fork/star` 判断它是否被复制;`issue/star` 不直接放进同一条轴,而是作为使用摩擦和维护压力的标记叠加上去。本文可粗略把 `stars/day` 高于 p75(~120/day)视为高传播速度,`fork/star` 高于 p75(~14%)视为高复制压力,`issue/star` 高于 p75(~1.4%)视为高摩擦信号。

| 信号组合 | 更像什么 | 读法 |
|---|---|---|
| 高 `stars/day` + 高 `fork/star` | 可复制工具/模板 | 不只被收藏,也被拿去改、跑、部署或二创 |
| 高 `stars/day` + 低 `fork/star` | attention object / meme / reading list | 传播强,但需要更多证据证明真实使用和工程质量 |
| 高 `stars/day` + 高 `issue/star` | 高需求 + 高摩擦 | 可能是真实使用带来大量问题,也可能是维护压力或产品不稳定 |
| 低 stars + 高 PR/contributor/activity | 深水区项目 | 当前 2000+ stars 样本难以覆盖,需要另抓低星高活跃仓库 |
...
| Media & 3D Generation | 37 | ~3.4k | ~6k | ~25k | ~25k | ~205k |
...
**读法**:这里按覆盖 stars 计算,不能按互斥分摊理解。B 系 skills/prompts 的 median 和 p95 高于 D/F 模型、媒体生成与 3D 层;A/B/C 的高星覆盖量说明关注度已经转向“把模型能力分发给 agent 的层”。翻译成产品形态,就是模型本身退到背景里,开发者更愿意传播那些能让模型进入工作流的接口、配置、skill 和 dashboard。median 使用常规中位数;p75/p95 使用 nearest-rank 口径。
