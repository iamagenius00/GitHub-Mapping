...
- **stars 是关注度指标**:它不能直接代表质量、用户数或严格增长率。正文中的 stars 多用近似展示,避免把个位数变化解读成趋势。
...
下文会用三个信号辅助理解高星项目:`stars/day` 看传播速度,`fork/star` 看复制压力,`issue/star` 看使用摩擦和维护压力。粗略来说,`stars/day` 超过 ~120/day 属于高传播,`fork/star` 超过 ~14% 属于高复制压力,`issue/star` 超过 ~1.4% 则提示项目可能已经遇到较多使用问题或维护请求。这些阈值不是质量评分,只是帮助区分不同类型的高星增长。

| 指标剖面 | 可能对应的传播机制 | 分析含义 |
|---|---|---|
| 高 `stars/day` + 高 `fork/star` | 可复制工具/模板 | 不只被收藏,也被拿去改、跑、部署或二创 |
| 高 `stars/day` + 低 `fork/star` | attention-heavy / README-first repo | 传播强,但真实使用和工程成熟度需要更多证据 |
...
按 `stars_per_day` 看,GitHub 关注度的早期窗口很短:新仓如果前期没有进入可见传播链路,后续自然抬升会更难。严格增长曲线需要完整的 stars 时间序列;本文的 `stars_per_day` 只作为平均速率参考。
