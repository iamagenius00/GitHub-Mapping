# GitHub AI 生态 6 个月观察 — multi-label

**版本**: 2026-05-11  
**窗口**: `created:2025-11-09..2026-05-09 stars:>=2000`, 703 个仓库  
**分类口径**: 28 个标签;每个仓库 1 个主类 + 0-2 个副类  
**配套材料**: 分类结果、标签说明、全量分类表见文末
...
703 个 2000+ 星新仓库里,按 multi-label / 标签覆盖口径,**A 基础设施覆盖 248 个仓库(35.3%)**,**B skills/prompts 覆盖 235 个仓库(33.4%)**,A 或 B 任一覆盖 **417 个仓库(59.3%)**。模型/训练 D 系只覆盖 **29 个仓库(4.1%)**;D/F 模型或多模态生成合计覆盖 **65 个仓库(9.2%)**。

关注度也沿着这条线集中:Top 20 仓库拿走 **~1.83M stars(29.4%)**,其中 **17/20** 被 A/B 标签覆盖,**13/20** 被 B 系(skills/prompts/config)标签覆盖。更重要的是,这些项目已经越过“收藏”层面:Top 20 的 median `stars/day` 约 **1.1k**;`fork/star`(forks/stars) median 约 **11.5%**。GitHub 这半年最显眼的竞争点变成了:谁能把 Claude Code / OpenClaw / Codex 包装成可分发、可安装、可复制的工作流。
...
- **stars 是关注度指标**:它不能直接代表质量、用户数或严格增长率。但这些仓库都在同一个 6 个月窗口内创建,抓取时点 stars 可以粗略反映这半年哪些项目获得关注。正文中的 stars 多用近似展示;精确值见原始数据。
- **为什么用 multi-label**:很多项目天然是复合形态,例如“skill 包 + research app”“agent harness + memory”“dashboard + observability”。多标签能保留这些交叉特征。
...
| **F** Multimodal Generation | 37 | 5.3% | ~205k | 3.3% |
...
- **D/F 任一覆盖 65 个仓库(9.2%)**:模型、训练和多模态生成仍然存在,但 GitHub 新仓关注度的重心已经转向上层工具和分发形态。
...
| **D/F** Models & Multimodal | 模型、训练、多模态生成与 3D/spatial | D1 training; D2 model architecture; D5 security disclosure; F3 media generation; F4 3D/spatial |
...
| Top 20 星标 | 占全数据集 | A/B 覆盖 | B 系覆盖 | median ★/day | median fork/star |
|---|---|---|---|---:|---:|
| ~1.83M | 29.4% | 17/20 | 13/20 | ~1.1k | 11.5% |

`fork/star` 不能严格等同于使用率,但可以粗略区分“收藏”与“复制、改装、接入”。Top 20 里有 13 个仓库的 `fork/star` 超过 10%,说明这批项目的传播已经越过 GitHub Trending 页面。
...
| 方向 | 覆盖仓库 | median | p75 | p95 | max | 覆盖 stars |
|---|---:|---:|---:|---:|---:|---:|
| Agent Infrastructure | 248 | ~4.2k | ~9.9k | ~42k | ~370k | ~2.97M |
| Skills & Prompts | 235 | ~4.6k | ~12k | ~48k | ~370k | ~3M |
| Vertical Applications | 200 | ~4.5k | ~12k | ~32k | ~92k | ~1.93M |
| Models & Training | 29 | ~3.3k | ~4.9k | ~10k | ~12k | ~119k |
| Multimodal Generation | 37 | ~3.4k | ~6k | ~25k | ~25k | ~205k |
| Education | 44 | ~3.8k | ~5.7k | ~17k | ~24k | ~247k |
| Hardware | 9 | ~3.6k | ~5.4k | ~29k | ~29k | ~73k |
| Fun-Utility | 77 | ~4k | ~6.9k | ~26k | ~46k | ~512k |
| AI Security Tools | 10 | ~2.8k | ~6.1k | ~176k | ~176k | ~214k |
| Eval & Observability | 17 | ~4.7k | ~19k | ~26k | ~26k | ~162k |
| Long-tail | 106 | ~3.6k | ~5.4k | ~12k | ~16k | ~477k |

**读法**:这里按覆盖 stars 计算,不能按互斥分摊理解。B 系 skills/prompts 的 median 和 p95 高于 D/F 模型与多模态层;A/B/C 的高星覆盖量说明关注度已经转向“把模型能力分发给 agent 的层”。翻译成产品形态,就是模型本身退到背景里,开发者更愿意传播那些能让模型进入工作流的接口、配置、skill 和 dashboard。median 使用常规中位数;p75/p95 使用 nearest-rank 口径。

### 3.3 现象级仓库说明什么

| 仓库 | 标签 | stars | ★/day | forks | fork/star |
|---|---|---:|---:|---:|---:|
| `openclaw/openclaw` | A1+B1 | ~370k | ~2.2k | ~76k | 20.6% |
| `ultraworkers/claw-code` | A1 | ~191k | ~5k | ~110k | 57.6% |
| `affaan-m/everything-claude-code` | B1+A2+J1 | ~176k | ~1.6k | ~27k | 15.5% |
| `forrestchang/andrej-karpathy-skills` | B2+B1 | ~121k | ~1.2k | ~12k | 10.1% |
...
更深一层看,这批高星项目并不是零散出现。很多项目围绕同一个上游、同一个文件格式或同一个命名入口成批冒出。下表是按仓库名、README、简介和 topics 做的关键词线索统计,不是互斥分类;它更适合用来看“一个概念扩散成生态”的速度。
...
这张表比单看 Top 20 更能解释为什么高星增长会这么快:一个概念先通过核心仓库获得注意力,随后同一批开发者会迅速补齐生态位,包括 awesome list、hosted wrapper、兼容层、教程、dashboard、插件、技能包和垂直版本。GitHub 上的新 AI 项目越来越像“围绕标准/上游的同步扩散”,而不是一个个互不相干的应用。

---

## 4. 发现二:窗口外上游塑造窗口内爆发

6 个月窗口只收“新建仓库”,但 agent 生态的真实引力中心并不总在窗口内。不少窗口内的高星仓库,其实是在给窗口外官方/半官方平台做 skill、wrapper、dashboard、中文化、评测、复刻或工作流包装。

| 上游 / 基础项目 | 状态 | 生态作用 |
|---|---|---|
| `anthropics/skills` | 窗口外 | SKILL.md 事实标准;许多 B1 仓库隐式对位它。 |
| `openai/codex` | 窗口外 | Codex CLI 是 openai/skills、oh-my-codex、codex-plugin-cc 等项目的共同分发目标。 |
| `google-gemini/gemini-cli` | 窗口外 | Gemini CLI 被许多 skill/harness 仓库列为兼容目标,也是 Google 系 agent 工具的入口。 |
| `NousResearch/hermes-agent` | 窗口外 | Hermes 生态催生 gbrain、hermes web UI、Hermes 教程和 memory/workspace 周边。 |
| `openclaw/openclaw` | 窗口内但更像上游 | 数据集内最大的中心项目;openclaw skills、clawhub、OpenClaw connector、OpenClaw 教程和 fork 都围绕它扩散。 |

这轮观察可以理解为:上游 agent harness 和 skills 规范先出现,然后在 6 个月内被 markdown skill、agent workflow、观测工具和垂直应用迅速产品化、社区化。

---

## 5. 发现三:multi-label 让交叉形态可见

下表看的是标签共现:只要两个标签同时出现在同一个仓库的主类/副类里,就计数。它回答的问题从“哪个方向最大”转向“这些方向如何混在一起形成新的项目形态”。multi-label 口径的价值就在这里:它能看到 agent infra、skills/prompts 和垂直应用之间的真实重叠。
...
TypeScript 仍是 agent 生态的胶水语言;B 系里也有一部分仓库没有传统主要语言,它们的“代码”就是 Markdown。即使有 Python/TypeScript 外壳,核心可复制单元也常常是 SKILL.md、CLAUDE.md 或 DESIGN.md。
...
按 `stars_per_day` 看,GitHub 关注度的早期窗口很短:新仓如果前期没有进入可见传播链路,后续自然抬升会更难。严格增长曲线仍需要 star-history 数据验证。
...
1. `stars` 是关注度指标,不能直接代表质量。
2. 本文没有 star-history 曲线,不能严格计算增长率;`stars_per_day` 是平均速率。
3. 数据集只覆盖 `created:2025-11-09..2026-05-09 stars:>=2000`,窗口外上游项目仍然很重要,比如 `anthropics/skills`、`openai/codex`、`google-gemini/gemini-cli`。
4. 分类依赖标签定义和人工判断,边界类 K1/E3、A1/A2 仍然可能有个案争议。
5. 本文主统计按标签覆盖计数;同一仓库可以同时计入多个方向,所以跨类覆盖率和覆盖 stars 不应相加成 100%。
